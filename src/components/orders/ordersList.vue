...
export default {
  ...
  methods: {
    ...
    invoice_type(type) {
      let typeInvoiceData = this.hashed_invoice_data;
      this.invoice_data = [];
      for (let data in typeInvoiceData) {
        if (typeInvoiceData[data][0].purchaseType === type) {
          this.invoice_data.push(typeInvoiceData[data]);
        }
      }
    },
    invoice_customer(name) {
      ...
      this.invoice_data = [];
      for (let data in typeInvoiceData) {
        if (typeInvoiceData[data][0].customers === name) {
          this.invoice_data.push(typeInvoiceData[data]);
        }
      }
    },
