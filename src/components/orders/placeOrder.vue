...
export default {
  ...
  computed: {
    buy_order() {
      if (this.static_data.purchaseType === "buy") {
        return true;
      }
      return false;
    },
  ...
  beforeMount() {
    this.$http
      ...
      .then((data) => {
        ...
        this.customers = data.body.customers.sort((a,b) => (a.joinDate > b.joinDate) ? 1 : ((b.joinDate > a.joinDate) ? -1 : 0));
        this.labours = data.body.labours;
        this.stocks = data.body.stocks.sort((a,b) => (a.barCodeId > b.barCodeId) ? 1 : ((b.barCodeId > a.barCodeId) ? -1 : 0));
      });
  },
  components: {
    invoiceItemVue,
